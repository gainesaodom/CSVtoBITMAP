...
def create_bit_distribution(weight,width,height): 
    img = Image.new('RGB', (width, height), color='white')
    for y in range(height):
        for x in range(width):
            pixel_index = y * width + x
            pixel_value = int(weight[pixel_index] * 255/100)
...
def main():
    ...
    Your options are: 
    'I' [A bit distribution map over 100 reads of the area of the image.]
    'F' [A bitmap distribution over 100 reads of the entirety of the chip's storage.]
    ...
    '''
        ...
        if iterations == SINGLE : 
            ...
            print(f"Bitmap generated and saved as "+output_file+" in "+prevdir+"\BITMAPS.")
        ...
        # I or F execution block
            ...
            for i in range(1, iterations + 1) :
                if not os.path.exists(input_file+'_'+str(i)+'.csv'):
                    raise FileNotFoundError(f"File '"+input_file+"_"+str(i)+".csv' does not exist.")
            ...
            bit_distribution = create_bit_distribution(weight, bitmap_width, bitmap_height)
            ...
            print(f"Bit distribution generated and saved as "+output_file+" in "+prevdir+"\BITMAPS.")
